...
public class SimpleRecoveryJob implements Job {
  ...
  public void execute(JobExecutionContext context) throws JobExecutionException {
    System.out.println("我是dufy——testxiam 的额定时任务");
    JobKey jobKey = context.getJobDetail().getKey();

    // if the job is recovering print a message
    if (context.isRecovering()) {
      _log.info("SimpleRecoveryJob: " + jobKey + " RECOVERING at " + new Date());//在什么时候恢复
    } else {
      _log.info("SimpleRecoveryJob: " + jobKey + " starting at " + new Date());//在什么时候开始
    }
